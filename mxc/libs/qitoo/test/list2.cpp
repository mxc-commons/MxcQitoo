...
main()
{
...

    {
        ...
        s.clear();
        BOOST_TEST(test_attr("ab,cd,ef,gh", (char_ >> char_) / char_(','), s));
        BOOST_TEST(s == "ab,cd,ef,gh");
        ...
        s.clear();
        BOOST_TEST(test_attr("ab,cd,efg", (char_ >> char_) / char_(',') >> char_, s));
        BOOST_TEST(s == "ab,cd,efg");

        s.clear();
        BOOST_TEST(test_attr("ab::cd::efg", (char_ >> char_) / string("::") >> char_, s));
        BOOST_TEST(s == "ab::cd::efg");

        s.clear();
        BOOST_TEST(test_attr("ab::cd::ef::g", (char_ >> char_) / string("::"), s, false));
        BOOST_TEST(s == "ab::cd::ef");

        s.clear();
        BOOST_TEST(test_attr("ab::cd::ef::", (char_ >> char_) / string("::"), s, false));
        BOOST_TEST(s == "ab::cd::ef");
    }
...

    {
        ...
        v.clear();
        BOOST_TEST(test_attr("a,b,c,d", +alpha / char_, v));
        BOOST_TEST(7 == v.size() 
            && "a" == v[0] && "," == v[1]
            && "b" == v[2] && "," == v[3]
            && "c" == v[4] && "," == v[5]
...

    {
        ...
        v.clear();
        BOOST_TEST(test_attr("#a,#", ('#' >> -alpha) / char_(','), v));
        BOOST_TEST(3 == v.size() &&
            !!v[0] && 'a' == boost::get<char>(v[0]) 
                   && ',' == boost::get<char>(v[1]) && !v[2]);
        ...
        v2.clear();
        BOOST_TEST(test_attr("#a,#", ('#' >> -alpha) / char_(','), v2));
        BOOST_TEST(2 == v2.size() && 'a' == v2[0]);
    }
